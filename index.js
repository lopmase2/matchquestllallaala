...
class BananaBot {
      ...
      if (eligibleBananas.length > 0) {
        ...
        if (bestBanana.banana_id === currentEquipBananaId) {
          this.log(
            colors.green(
              `Đang sử dụng quả chuối tốt nhất: ${colors.yellow(
                ...
                bestBanana.sell_exchange_peel
              )} Peels / ${colors.yellow(bestBanana.sell_exchange_usdt)} USDT.`
            )
          );
          ...
          if (bestBanana.sell_exchange_usdt >= 0.1) {
            this.log(
              colors.red(
                `Đã đạt mục tiêu! Giá trị USDT của chuối: ${colors.yellow(
                  bestBanana.sell_exchange_usdt
                )} USDT`
              )
            );

            const filePath = path.join(__dirname, "banana.txt");
            const newBananaInfo = `Account ${accountIndex + 1}: ${
              bestBanana.name
            } - ${bestBanana.sell_exchange_usdt} USDT`;
      ...
      try {
        ...
        if (response.data.code === 0) {
          ...
          if (lotteryInfo.countdown_end === true) {
            this.log(
              colors.yellow("Countdown kết thúc. Đang claim lottery...")
            );
      ...
      try {
        ...
        try {
          ...
          if (remainingTimeMinutes <= 0) {
            ...
            remainLotteryCount =
              (updatedLotteryInfoData.data || {}).remain_lottery_count || 0;
            remainingTimeMinutes = this.calculateRemainingTime(
          ...
          if (speedup > 0) {
            ...
            this.log(
              colors.yellow(`Thực hiện speedup tối đa ${maxSpeedups} lần...`)
            );
            const speedupLotteryInfo = await this.doSpeedup(maxSpeedups);
            if (speedupLotteryInfo) {
              remainingTimeMinutes =
                this.calculateRemainingTime(speedupLotteryInfo);
            }
          }
          ...
          if (remainLotteryCount > 0) {
            ...
            for (let i = 0; i < remainLotteryCount; i++) {
              this.log(
                `Đang harvest lần thứ ${i + 1}/${remainLotteryCount}...`
              );
              ...
              if (doLotteryResponse.status === 200) {
                ...
                const sellExchangePeel =
                  lotteryResult.sell_exchange_peel || "N/A";
                const sellExchangeUsdt =
                  lotteryResult.sell_exchange_usdt || "N/A";

                this.log(`Harvest thành công ${bananaName}`);
                console.log(
                  colors.yellow(`     - Banana Name : ${bananaName}`)
                );
                console.log(
                  colors.yellow(
                    `     - Peel Limit : ${
                ...
                this.log(
                  colors.red(
                    `Lỗi không mong muốn khi harvest lần thứ ${i + 1}.`
                  )
                );
              }
            }
        ...
        if (todayClickCount < maxClickCount) {
          ...
          if (clickCount > 0) {
            ...
            for (let i = 0; i < 9; i++) {
              const part =
                Math.floor(Math.random() * (remaining / (10 - i))) * 2;
              parts.push(part);
              remaining -= part;
            }
            ...
            for (const part of parts) {
              ...
              if (response && response.code === 0) {
                ...
                this.log(
                  colors.magenta(
                    `Nhận được ${peel} Peel, ${speedup} Speedup...`
                  )
                );
              } else {
            ...
            if (updatedSpeedup > 0) {
              this.log(
                colors.yellow(
                  `Thực hiện speedup, bạn có ${updatedSpeedup} lần...`
                )
              );
        ...
        if (doQuests) {
          try {
            ...
            const isClaimedQuestLottery =
              questListData.data.is_claimed || false;
  ...
  async main() {
    ...
    //   .split("\n")
    //   .filter(Boolean);
    const doQuestsAnswer = await this.askQuestion(
    ...
    while (true) {
      ...
      for (let i = 0; i < userData.length; i++) {
        ...
        try {
          ...
          console.log(
            `Lỗi proxy: ${error.message}. Chuyển sang tài khoản tiếp theo.`
          );
        }
